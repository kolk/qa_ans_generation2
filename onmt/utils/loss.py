...
def build_loss_compute(model, tgt_vocab, src_vocab, opt, train=True):
    ...
    if opt.copy_attn:
        compute = onmt.modules.CopyGeneratorLossCompute(
            model.generator, tgt_vocab, src_vocab, opt.copy_attn_force,
...
class LossComputeBase(nn.Module):
    ...
    def monolithic_compute_loss(self, batch, output, attns, train):
        ...
        #logger.info("shard_state_valid")
        #logger.info(shard_state_valid.__dict__)
    ...
    def sharded_compute_loss(self, batch, output, attns,
        ...
        for shard in shards(shard_state, shard_size):
            #logger.info(shard.keys())
            #logger.info("shard")
            #logger.info("copy_attn size")
            #logger.info(shard["copy_attn"].size())
            #logger.info("output size")
            #logger.info(shard["output"].size())
            #logger.info("targer size")
            #logger.info(shard["target"].size())
            #logger.info("align size")
            #logger.info(shard["align"].size())
