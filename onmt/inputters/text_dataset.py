...
class TextDataset(DatasetBase):
    ...
    def __init__(self, fields, src_examples_iter, tgt_examples_iter, ans_examples_iter,
                 num_src_feats=0, num_tgt_feats=0, num_ans_feats=0,
                 src_seq_length=0, tgt_seq_length=0, ans_seq_length=0,
        ...
        self.n_ans_feats = num_ans_feats
        ...
        def filter_pred(example):
            ...
            return 0 < len(example.src) <= src_seq_length \
                ...
                and 0 < len(example.ans) <= ans_seq_length
    ...
    def sort_key(self, ex):
        ...
        if hasattr(ex, "tgt"):
            if hasattr(ex, "ans"):
                return len(ex.src), len(ex.ans), len(ex.tgt)
            else:
                return len(ex.src), len(ex.tgt)
        else:
            if hasattr(ex, "ans"):
                return len(ex.src), len(ex.ans)
            else:
                return len(ex.src)
    ...
    @staticmethod
    def get_fields(n_src_features, n_tgt_features, n_ans_features):
        """
        Args:
        ...
        def make_src(data, vocab):
            """ ? """
            '''
            logger.info("make_src function")
            logger.info('caller name:')
            ...
            for i, sent in enumerate(data):
                if sent.size(0) > m:
                    ...
                    m_sent = sent
                #logger.info(i)
                #logger.info(sent.size())
                #logger.info(sent)
            logger.info("max sent size " + str(m) + " in index " + str(m_i))
            logger.info(m_sent)
            '''
    ...
    def _dynamic_dict(self, examples_iter):
        for example in examples_iter:
            ...
            ############# Modified ###################
            ans = example["ans"]
            src_vocab = torchtext.vocab.Vocab(Counter(src+ans),
                                              specials=[UNK_WORD, PAD_WORD])
            ###########################################
            self.src_vocabs.append(src_vocab)
            # Mapping source tokens to indices in the dynamic dict.
            logger.info("len src map ")
            logger.info(len([src_vocab.stoi[w] for w in src]+ [src_vocab.stoi[w] for w in ans]))
