...
from onmt.utils.logging import logger as logger_
...
class Translator(object):
    ...
    def _translate_batch(self, batch, data):
        ...
        dec_states = self.model.decoder.init_decoder_state(
            src, ans, memory_bank, memory_bank_ans, enc_states, enc_states_ans)
        ...
        if ans_lengths is None:
            ans_lengths = torch.Tensor(batch_size).type_as(memory_bank_ans.data)\
                                                  .long()\
                                                  .fill_(memory_bank_ans.size(0))
        #########################################

        #### TODO: BEAM SEARCH ANS INCORPORATION ####
        ...
        ########## Modified ####################
        memory_bank_ans = rvar(memory_bank_ans.data)
        memory_lengths_ans = ans_lengths.repeat(beam_size)
        ###########################################
    ...
    def _run_target(self, batch, data):
        ...
        dec_states = \
            self.model.decoder.init_decoder_state(src, ans, memory_bank, memory_bank_ans, enc_states, enc_states_ans)
