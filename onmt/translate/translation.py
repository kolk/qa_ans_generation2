...
class TranslationBuilder(object):
    ...
    def _build_target_tokens(self, src, ans, src_vocab, src_raw, src_ans, pred, attn):
        ############### Modified ######################
        ...
        for tok in pred:
            ...
            if tokens[-1] == inputters.EOS_WORD:
                tokens = tokens[:-1]
                break
        if self.replace_unk and (attn is not None) and (src is not None) and (ans is not None):
    ...
    def from_batch(self, translation_batch):
        ...
        if data_type == 'text':
            ...
            ans = batch.ans[0].data.index_select(1, perm)
        else:
            src = None
