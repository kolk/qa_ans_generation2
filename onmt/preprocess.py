...
def build_save_in_shards(src_corpus, tgt_corpus, ans_corpus, fields,
                         corpus_type, opt):
    """
    ...
    tgt_iter = inputters.ShardedTextCorpusIterator(
        ...
        assoc_iter=src_iter)
    ans_iter = inputters.ShardedTextCorpusIterator(
        ans_corpus, opt.ans_seq_length_trunc,
    ...
    while not src_iter.hit_end():
        ...
        dataset = inputters.TextDataset(
            fields, src_iter, tgt_iter, ans_iter,
            src_iter.num_feats, tgt_iter.num_feats, ans_iter.num_feats,
...
def build_save_dataset(corpus_type, fields, opt):
    ...
    if opt.data_type == 'text':
        return build_save_in_shards(
            src_corpus, tgt_corpus, ans_corpus, fields,
            corpus_type, opt)
